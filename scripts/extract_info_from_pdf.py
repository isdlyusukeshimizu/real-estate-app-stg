...
- 認識された登記所名は、そのままの文字で出力してください（例：「東京法務局」「東近江(出)」など）。
- ただし、「登記部門」と書かれている場合、または登記所名が空白・読み取れない場合は、以下の文言を出力してください：
  →「「登記部門」の記載、または空白のため特定できませんでした」
- 出力は必ず一行だけ、登記所名または上記の文言のみとしてください。
- 余計な説明や記号は出力しないでください。
