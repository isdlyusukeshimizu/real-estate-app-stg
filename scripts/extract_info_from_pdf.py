...
- 通常は「〇〇法務局」「〇〇法務局〇〇支局」「〇〇出張所」のような形式で登記所名が記載されています。
- 「東近江(出)」のように括弧付きの場合は、「(出)」を「支局」、「(本)」を「本局」と読み替えて正式名称を出力してください。
- ただし、「登記部門」としか書かれていない場合は、登記所名が省略されているため、次の文言を出力してください：
  →「登記部門という記載のため省略されております」
- 出力は必ずその登記所名のみ、または上記の特別文言のみ。
- 余計な説明文や記号、接頭語、接尾語は出力しないでください。
- 出力は一行だけ、名前または文言だけにしてください。
